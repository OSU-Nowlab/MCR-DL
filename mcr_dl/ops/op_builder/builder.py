...
try:
    ...
    print(f"{WARNING} unable to import torch, please install it if you want to pre-compile any mcr_dl ops.")
else:
...
def installed_cuda_version(name=""):
    ...
    if "CUDA_HOME" in os.environ:
        ...
        cuda_home = os.environ.get("CUDA_ROOT")
    assert cuda_home is not None and os.path.exists(cuda_home), "CUDA_HOME does not exist, unable to compile CUDA op(s)"
...
# to build mcr_dl and system-wide installed cuda 11.2
...
class OpBuilder(ABC):
    ...
    def absolute_name(self):
        '''
        Returns absolute build path for cases where the op is pre-installed, e.g., mcr_dl.ops.adam.cpu_adam
        will be installed as something like: mcr_dl/ops/adam/cpu_adam.so
        '''
        pass
    ...
    def mcr_dl_src_path(self, code_path):
    ...
    def jit_load(self, verbose=True):
        ...
        sources = [self.mcr_dl_src_path(path) for path in self.sources()]
        extra_include_paths = [self.mcr_dl_src_path(path) for path in self.include_paths()]
